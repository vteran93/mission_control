#!/usr/bin/env python3
"""
Jarvis-PM Direct Responder - Responde directamente via clawdbot agent
"""
import sys
import os
import requests
...
AGENT_NAME = 'Jarvis-PM'
PROCESSED_FILE = BASE_DIR / 'daemon/state/pm-processed-messages.txt'
...
def check_messages_for_me():
    ...
    try:
        ...
        for msg in messages:
            ...
            if 'jarvis-pm' in content_lower or '@jarvis-pm' in content_lower:
...
import requests
requests.post('http://localhost:5001/api/messages', json={{'from_agent': 'Jarvis-PM', 'content': 'TU_RESPUESTA'}})"""
    
    try:
        result = subprocess.run(
            ['clawdbot', 'agent', '--agent', 'jarvis-pm', '--message', prompt, '--local'],
            capture_output=True,
            text=True,
            timeout=60
        )
