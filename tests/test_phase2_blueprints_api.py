...
def test_import_blueprint_persists_records(configured_app_with_specs):
    ...
    assert payload["certified_input"]["architecture_synthesis"]["technical_contracts"]
    assert payload["certified_input"]["confidence_assessment"]["recommended_status"] == "ready_for_planning"
    assert payload["certified_input"]["question_budget"]["open_questions_count"] == 0
...
def test_blueprint_feedback_and_retrospective_are_persisted(configured_app_with_specs):
    ...
    assert payload["certified_input"]["contract_name"] == "mission_control_certified_input"
    assert payload["certified_input"]["architecture_synthesis"]["adr_bootstrap"]
    assert payload["certified_input"]["confidence_assessment"]["score"] >= 0
